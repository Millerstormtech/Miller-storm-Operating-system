...
class BranchManagerStormChatScreen extends StatefulWidget {
  const BranchManagerStormChatScreen({Key? key}) : super(key: key);

  @override
  _BranchManagerStormChatScreenState createState() => _BranchManagerStormChatScreenState();
}

class _BranchManagerStormChatScreenState extends State<BranchManagerStormChatScreen> {
  ...
  // Guided tour (StormChat, management view). The mobile list shows the same
  // controls as the rep view — compose a DM, groups, direct messages — so it
  // mirrors the web "storm-chat" tour. The "Create a group"/"Requests waiting"
  // management controls aren't present on this mobile screen, so those steps are
  // skipped. No persistent search box either. Auto-starts once per user/device.
  ...
  static const _tourSeenKey = 'tour_seen_bm_storm_chat_v1';
