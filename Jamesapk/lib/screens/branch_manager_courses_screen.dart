...
class BranchManagerCoursesScreen extends StatefulWidget {
  const BranchManagerCoursesScreen({super.key});

  @override
  State<BranchManagerCoursesScreen> createState() => _BranchManagerCoursesScreenState();
}

class _BranchManagerCoursesScreenState extends State<BranchManagerCoursesScreen> with SingleTickerProviderStateMixin {
  ...
  String? _userId;
  static const String _cacheKey = 'bm_courses_cache';
    ...
    try {
      ...
      if (cachedJson != null) {
        ...
        final List<dynamic> courses =
            (data is List ? data : []).where((c) => c['status'] == 'published').toList();
    ...
    for (int attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        if (response.statusCode == 200) {
          ...
          // The courses API returns drafts to leadership roles (for web course
          // management), but the app should only ever show PUBLISHED courses.
          courses = courses.where((c) => c['status'] == 'published').toList();
          courses.sort((a, b) {
  ...
  void _showAssignPlaylistDialog(Map<String, dynamic> playlist) async {
    ...
    // Fetch ALL sales users (company-wide) for playlist assignment.
  ...
  Widget _buildCoursesTab() {
    ...
    return Column(
      ...
      itemBuilder: (context, index) {
        // AI Assistants ("Jay's AI Clone") section rendered after the courses,
        // matching the Sales training screen.
