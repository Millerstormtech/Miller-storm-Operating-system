...
// Branch Manager dashboard: in this org the sales "teams" (Gunner, Cooper,
// Mike Muscari, ...) are led by branch managers. Each team lead IS the branch
// manager and appears as their own rep on the sales leaderboard. This lists
// those branch managers with their OWN individual sales for the window.
// Stays inside /admin. Nothing hardcoded here — the leaderboard flags each
// rep's isTeamLead using the org chart.
export function BranchManagerDashboard() {
  ...
  // Only the branch managers (team leads), ranked by their own revenue.
  const managers = useMemo(() => {
    return (rows || [])
      .filter((r) => r.isTeamLead)
      .sort((a, b) => b.revenue - a.revenue || b.won - a.won || b.verifiedKnocks - a.verifiedKnocks);
  }, [rows]);

  const totals = useMemo(() => {
    return {
      count: managers.length,
      revenue: managers.reduce((s, r) => s + (r.revenue || 0), 0),
      won: managers.reduce((s, r) => s + (r.won || 0), 0),
      knocks: managers.reduce((s, r) => s + (r.verifiedKnocks || 0), 0),
    };
  }, [managers]);
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 20 }}>
        <Stat label="Branch managers" value={String(totals.count)} color="#2563eb" />
        <Stat label={`Their revenue (${salesWindow})`} value={money(totals.revenue)} color="#0891b2" />
      ...
      <div className="panel">
        <div className="panel-header" style={{ fontWeight: 700 }}>🏆 Branch Managers — Sales ({WINDOWS.find((w) => w.key === salesWindow)?.label})</div>
        <div className="panel-body" style={{ padding: 0 }}>
          {error ? (
            ...
            <div style={{ padding: 40, textAlign: "center", color: "#9ca3af" }}>No branch manager sales for this period.</div>
          ) : (
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
                <thead>
                  <tr style={{ background: "#f9fafb", textAlign: "left", color: "#6b7280", fontSize: 12.5 }}>
                    ...
                    <th style={{ padding: "10px 16px", fontWeight: 600 }}>Branch Manager</th>
                    <th style={{ padding: "10px 16px", fontWeight: 600 }}>Branch</th>
                ...
                <tbody>
                  {managers.map((r, i) => (
                    <tr key={`${r.name}-${r.team}`} style={{ borderTop: "1px solid #f0f1f3" }}>
                      ...
                      <td style={{ padding: "10px 16px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          {r.headshotUrl ? (
                            ...
                            <div style={{ width: 34, height: 34, borderRadius: "50%", background: "#eef2ff", color: "#4338ca", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 12 }}>{initials(r.name)}</div>
                          )}
                          <span style={{ fontWeight: 600, color: "#111827" }}>{r.name}</span>
                        </div>
                      </td>
                      <td style={{ padding: "10px 16px", color: "#6b7280", whiteSpace: "nowrap" }}>{r.branch || "—"}</td>
                      <td style={{ padding: "10px 16px", color: "#6b7280", whiteSpace: "nowrap" }}>{r.team || "—"}</td>
